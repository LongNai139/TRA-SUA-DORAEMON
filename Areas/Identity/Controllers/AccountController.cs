...
namespace DIHA.Areas.Identity.Controllers
{
    [Authorize]
    [Area("Identity")]
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
             ...
             if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Nếu người tạo tài khoản là Admin
                    if (User.Identity != null && User.Identity.IsAuthenticated && User.IsInRole("Administrator"))
                    {
                        var staff = new DIHA.Areas.Staff.Models.Data.Staff
                        {
                            StaffName = model.UserName,
                            NormalizedStaffName = model.UserName.ToUpper(),
                        ...
                        // Cập nhật lại AppUser với IdStaff vừa tạo
                        user.IdStaff = staff.IdStaff;
                    ...
                    {
                        var custom = new DIHA.Areas.Custom.Models.Data.Custom
                        {
                            CustomName = model.UserName,
                            NormalizedCustomName = model.UserName.ToUpper(),
                            BirthDate = model.BirthDate,
                            Email = model.Email,
                            HomeAdress = model.HomeAdress,
                            PhoneNumber = model.PhoneNumber,
                        ...
                        // Lưu Custom vào database
                        _context.Customs.Add(custom);
                        await _context.SaveChangesAsync();

                        // Cập nhật lại AppUser với IdCustom vừa tạo
                        user.IdCustom = custom.IdCustom;
                        await _userManager.UpdateAsync(user);
                    }
                        // Gửi email xác nhận
